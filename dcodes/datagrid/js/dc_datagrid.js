...
TINY.table = function () {
	...
	sorter.prototype.init = function () {
		...
		for (i; i < t.w; i++) {
			...
			if (d) {
				var o = document.createElement('option');
				o.value = i;
				o.innerHTML = T$$('h5', c)[0].innerHTML;
				d.appendChild(o)
			}
		}
		this.reset()
	...
	sorter.prototype.search = function (f) {
		var i = x = n = 0,
			k = -1,
			q = T$(f).value.toLowerCase();
		if (this.p.colddid) {
			k = T$(this.p.colddid).value
		}
		var s = (k == -1) ? 0 : k,
			e = (k == -1) ? this.t.w : parseInt(s) + 1;
		for (i; i < this.t.l; i++) {
			var r = this.t.r[i],
				v;
			if (q == '') {
