...
$.fn.preloader = function(options){
		...
		timer = setInterval(function(){
			...
			for(i=0;i<images.length;i++)
			{
				if(images[i].complete==true)
				{
					...
					function(){ $(this).parent().removeClass("dc_preloader"); });
	...
	images.each(function(){
		...
		$(this).wrap("<a class='dc_preloader' />");
		else
		$(this).parent().addClass("dc_preloader");
	...
	var icon = jQuery("<img />",{		
		id : 'loadingicon',
		src : 'dcodes/image_preloader/css/loading.gif'  // relative to html document calling from
	}).hide().appendTo("body");
